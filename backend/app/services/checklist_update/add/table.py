...
_ALLOWED_FIELDS = frozenset(
    {"type", "label", "humanReadableId", "columns", "rows"}
)
...
def add_table(checklist: dict, operation: AddComponentOperation) -> dict:
    ...
    if payload.get("type") != "table":
        raise InvalidComponentPayloadError(
            f"table: expected type 'table', got {payload.get('type')!r}"
        )

    reject_unknown_fields(payload, _ALLOWED_FIELDS, "table")
    label = require_str(payload, "label", "table")

    raw_columns = payload.get("columns", [])
    if not isinstance(raw_columns, list):
        raise InvalidComponentPayloadError("table: 'columns' must be a list")
    columns = [_validate_column(col, i) for i, col in enumerate(raw_columns)]
    ...
    rows = [_validate_row(row, i, columns) for i, row in enumerate(raw_rows)]

    component: dict = {
        "id": generate_component_id("table"),
        "type": "table",
        "label": label,
        "columns": columns,
        "rows": rows,
